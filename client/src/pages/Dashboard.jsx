...
export default function Dashboard({ orders, drivers }) {
  ...
  const avgEta = orders.length ? Math.round(orders.reduce((s, o) => s + (o.eta || 0), 0) / orders.length) : 0
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '340px 1fr', height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{ background: 'var(--surface)', borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 1, background: 'var(--border)', borderBottom: '1px solid var(--border)' }}>
          {[
            { val: active.length,  label: 'Active',     color: 'var(--accent2)' },
      ...
      <div style={{ display: 'flex', flexDirection: 'column', position: 'relative' }}>
        ...
        <div style={{ padding: '12px 20px', borderTop: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 12, background: 'var(--surface)' }}>
          <button onClick={() => setShowNew(true)} style={{ fontFamily: 'var(--font)', fontSize: 13, fontWeight: 600, padding: '9px 18px', borderRadius: 8, border: 'none', cursor: 'pointer', background: 'var(--accent)', color: '#fff', transition: 'all 0.2s', whiteSpace: 'nowrap' }}>
            + New Order
          </button>
      ...
      {/* NEW ORDER MODAL */}
      {showNew && (
        <div onClick={() => setShowNew(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 16, padding: 28, width: 420 }}>
            ...
            <form onSubmit={createOrder} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {[
                ...
                <input key={key} required placeholder={placeholder} value={form[key]}
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 8 }}>
                <button type="button" onClick={() => setShowNew(false)} style={{ flex: 1, padding: '10px', borderRadius: 8, border: '1px solid var(--border)', background: 'transparent', color: 'var(--text)', cursor: 'pointer', fontFamily: 'var(--font)' }}>Cancel</button>
                <button type="submit" style={{ flex: 2, padding: '10px', borderRadius: 8, border: 'none', background: 'var(--accent)', color: '#fff', cursor: 'pointer', fontFamily: 'var(--font)', fontWeight: 600 }}>Create Order</button>
              </div>
            </form>
          </div>
        </div>
